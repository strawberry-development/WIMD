...
class WindProgressBar
{
    ...
    private ?ConsoleFormatter $consoleHelper = null;
    ...
    public function __construct(OutputInterface $output, int $max = 0, float $minSecondsBetweenRedraws = 1 / 25)
    {
        ...
        $this->consoleHelper = new ConsoleFormatter();
    }

    /**
     * Sets a placeholder formatter for a given name, globally for all instances of WindProgressBar.
     *
     * This method also allow you to override an existing placeholder.
     *
     * @param string                       $name     The placeholder name (including the delimiter char like %)
     * @param callable(WindProgressBar):string $callable A PHP callable
     */
    ...
    public function setFormat(string $bar, string $information): void
    {
        $this->format = null;
        $this->internalFormat = "";
        $this->informationTemplate = $information;
        $this->barTemplate = $bar;
    }
    ...
    /**
     * Sets whether to overwrite the WindProgressBar, false for new line.
     */
    ...
    public function display(): void
    {
        ...
        // Process bar and information templates
        $processedBar = $this->buildLine($this->barTemplate);
        $processedInfo = $this->buildLine($this->informationTemplate);

        // Generate dotted line with processed content
        $formattedLine = "  " .  $this->consoleHelper->formatLine(
                "$processedBar",
                "$processedInfo"
            );

        $this->overwrite($formattedLine);
    }
    ...
    private function buildLine($template): string
    {
        ...
        $line = preg_replace_callback($regex, $callback, $template);
